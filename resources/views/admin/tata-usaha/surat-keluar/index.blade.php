            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex space-x-2">
                        {{-- <a href="{{ route('admin.surat-keluar.export') }}"
                ...
                <!-- Search and filter -->
                <div class="mb-6">
                    <form action="{{ route('admin.tata-usaha.surat-keluar.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                        ...
                        <div>
                            ...
                            <select name="klasifikasi" id="klasifikasi"
                                ...
                                @foreach(\App\Models\KlasifikasiSurat::all() as $k)
                                    <option value="{{ $k->id }}" {{ request('klasifikasi') == $k->id ? 'selected' : '' }}>
                                        {{ $k->kode }} - {{ $k->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-4 flex justify-end space-x-2">
                            ...
                            <a href="{{ route('admin.tata-usaha.surat-keluar.index') }}" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-xs leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <table class="min-w-full bg-white divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sifat</th>
                            ...
                            @forelse($suratKeluar as $index => $item)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ...
                                        <span class="ml-1 text-xs">{{ $item->klasifikasiSurat->nama }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('admin.tata-usaha.surat-keluar.show', $item->id) }}"
                                            ...
                                            <a href="{{ route('admin.tata-usaha.surat-keluar.edit', $item->id) }}"
                                               class="text-yellow-600 hover:text-yellow-900" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.tata-usaha.surat-keluar.destroy', $item->id) }}" method="POST" class="inline"
