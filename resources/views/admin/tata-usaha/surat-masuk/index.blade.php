<x-layouts.admin title="Surat Masuk" subtitle="Kelola data surat masuk">

    {{-- Breadcrumb --}}
    <x-main.layouts.breadcrumb :items="[['title' => 'Tata Usaha', 'url' => '#'], ['title' => 'Surat Masuk']]" />
        ...
        <div class="overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900">Data Surat Masuk</h3>
                        <p class="text-sm text-gray-600">Kelola semua surat masuk yang diterima</p>
                    </div>
                    <div class="flex items-center space-x-2 text-sm text-gray-500">
                        <span>Menampilkan <span id="visibleCount">{{ $suratMasuk->count() }}</span> dari
                            {{ $suratMasuk->total() }} data</span>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex-1 max-w-md">
                        <div class="relative">
                            ...
                            <form method="GET" action="{{ route('admin.tata-usaha.surat-masuk.index') }}">
                    ...
                    <div class="flex items-center space-x-3">
                        <x-main.buttons.action-button href="{{ route('admin.tata-usaha.surat-masuk.create') }}"
                            variant="primary">
                ...
                <div id="advanced-filters"
                    class="mt-4 {{ request()->anyFilled(['dari', 'tanggal_mulai', 'tanggal_akhir']) ? '' : 'hidden' }}">
                    <form action="{{ route('admin.tata-usaha.surat-masuk.index') }}" method="GET"
                        class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input type="hidden" name="search" value="{{ request('search') }}">
                        <div>
                            <label for="dari"
                                class="block text-sm font-medium text-gray-700 mb-1">Dari/Pengirim</label>
            ...
            {{-- Filter Info --}}
            @if (request()->anyFilled(['search', 'dari', 'tanggal_mulai', 'tanggal_akhir']))
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($suratMasuk as $index => $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    @if ($item->scanning)
                                        <a href="{{ route('admin.tata-usaha.surat-masuk.download', $item->id) }}"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <x-main.datatables.action-buttons :show-route="route('admin.tata-usaha.surat-masuk.show', $item->id)" :edit-route="route('admin.tata-usaha.surat-masuk.edit', $item->id)"
                                        :delete-route="route('admin.tata-usaha.surat-masuk.destroy', $item->id)" :item-name="'surat masuk ' . $item->no_surat" />
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8"
                                    ...
                                    <div class="flex flex-col items-center py-8">
                                        ...
                                        <p class="text-gray-500">Data surat masuk tidak ditemukan</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            @if ($suratMasuk->hasPages())
                <div class="mt-6 border-t border-gray-200 pt-4">
                    <x-main.datatables.pagination :paginator="$suratMasuk" />
                </div>
            @endif
        </div>
    </x-main.cards.content-card>
